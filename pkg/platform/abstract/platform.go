...
func (ap *Platform) GetProcessorLogsAndBriefError(scanner *bufio.Scanner) (string, string) {
	...
	for scanner.Scan() {
		...
		if err != nil {
			...
			// if there's a panic stop appending raw log lines to the briefErrorsMessage (unnecessary information from now on)
			// the panic can still be seen in the full logs
			if strings.HasPrefix(rawLogLine, "panic: Wrapper") {
				reachedPanicLine = true
			}
